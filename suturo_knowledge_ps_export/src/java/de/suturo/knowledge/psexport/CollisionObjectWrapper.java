...
public class CollisionObjectWrapper {
    ...
    /**
     * Simple method to create a Collision Object message which removes the object with the given id
     * 
     * @param id
     *            Unique (in ROS instance) ID
     ...
     * @param quat
     */
    public void addPose(double posX, double posY, double posZ, double roll, double pitch, double yaw) {
	if (poses == null) {
    ...
    private static <T> T check(T in, String field) throws IncompleteValueException {
	if (in == null) {
    ...
    /**
     * Generates a {@link CollisionObject} message while performing sanity checks, assuring completeness.
     * 
	...
	try {
	    ...
	    throw new IllegalStateException("Missing attribute: " + e.getMissing());
	}
	return co;
    }
