...
public class WekaClassifier implements ObjectClassifier {
	...
	if (arff == null) {
	    throw new FileNotFoundException("Could not get arff file " + ARFF_FILE + " from classpath!");
	}
	...
	this.dataSet = data;
	classifier = (Classifier) SerializationHelper.read(WekaClassifier.class.getResourceAsStream(CLASSIFIER_PATH));
	...
	try {
		...
		System.out.println("Class " + inst.classAttribute().value(i) + " has probability " + dist[i]);
	    }
    ...
    private static void setPOValue(PerceivedObject po, Instance inst, Attribute att) {
	...
	double middleLen = Math.min(Math.max(po.matched_cuboid.length1, po.matched_cuboid.length2),
		Math.max(po.matched_cuboid.length2, po.matched_cuboid.length3));
