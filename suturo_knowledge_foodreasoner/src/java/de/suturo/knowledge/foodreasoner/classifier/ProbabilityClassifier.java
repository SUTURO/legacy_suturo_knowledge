...
public class ProbabilityClassifier implements ObjectClassifier {
	...
	public String classifyPerceivedObject(PerceivedObject percObject) {
		...
		for (String inst : instanceNames) {
			probRes.add(new ProbabilityObject(inst, gauss(
					Math.sin(percObject.c_color_average_h * Math.PI / 180),
					probData.get(inst + "_hue_sin_mean"),
					probData.get(inst + "_hue_sin_sd"))
					+ gauss(Math.sin(percObject.c_color_average_h * Math.PI
							/ 180), probData.get(inst + "_hue_cos_mean"),
							probData.get(inst + "_hue_cos_sd"))
					+ gauss(maxl / midl,
							probData.get(inst + "_length_relation_1_mean"),
							probData.get(inst + "_length_relation_1_sd"))
					+ gauss(maxl / minl,
							probData.get(inst + "_length_relation_2_mean"),
