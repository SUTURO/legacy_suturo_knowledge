...
public class ProbabilityClassifier implements ObjectClassifier {

    private final String resultPrefix = "http://www.suturo.de/ontology/hierarchy#";
    private final HashMap<String, Double> probData;
    ...
    /**
     * Classifies an object given some values that were calculated by the perception
     * 
	...
	for (String inst : instanceNames) {
	    probRes.add(new ProbabilityObject(inst, gauss(Math.sin(percObject.c_color_average_h * Math.PI / 180),
		    probData.get(inst + "_hue_sin_mean"), probData.get(inst + "_hue_sin_sd"))
		    + gauss(Math.sin(percObject.c_color_average_h * Math.PI / 180),
			    probData.get(inst + "_hue_cos_mean"), probData.get(inst + "_hue_cos_sd"))
		    + gauss(maxl / midl, probData.get(inst + "_length_relation_1_mean"),
			    probData.get(inst + "_length_relation_1_sd"))
		    + gauss(maxl / minl, probData.get(inst + "_length_relation_2_mean"),
			    probData.get(inst + "_length_relation_2_sd"))));
	}
    ...
    /**
     * Calculates the probability density of a value given mean and standard deviation
     * 
    ...
    /**
     * Returns a String filled with detailed information about the last classification
     * 
